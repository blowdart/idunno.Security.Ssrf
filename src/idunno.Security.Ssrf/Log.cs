...
{
    [LoggerMessage(EventId = 1, Level = LogLevel.Warning, Message = "Connection to {uri} blocked as it evaluated unsafe.")]
    public static partial void UnsafeUri(ILogger logger, Uri uri);

    [LoggerMessage(EventId = 2, Level = LogLevel.Warning, Message = "DNS resolution for {uri} threw an exception")]
    ...
    [LoggerMessage(EventId = 5, Level = LogLevel.Error, Message = "All resolved IP addresses for {uri} are unsafe.")]
    public static partial void AllResolvedIpAddressesUnsafe(ILogger logger, Uri uri);

    [LoggerMessage(EventId = 6, Level = LogLevel.Error, Message = "Some resolved IP addresses for {uri} are unsafe and failMixedResults is enabled.")]
